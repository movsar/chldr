...
namespace api_server.GraphQL.MutationResolvers
{
    public class EntryResolver
    {
        ...
        public EntryResolver(IDataProvider dataProvider, IExceptionHandler exceptionHandler)
        {
        ...
        public async Task<RequestResult> PromoteAsync(string userId, string entryId)
        {
            using var unitOfWork = (SqlDataAccessor)_dataProvider.Repositories(userId);
            unitOfWork.BeginTransaction();
            var entriesRepository = (SqlEntriesRepository)unitOfWork.Entries;
